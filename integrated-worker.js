...
async function commitExport(env) {
  ...
  const rawFiles = env.__exportFiles || [];
  const files = rawFiles.filter(f => f && typeof f.path === "string" && typeof f.content === "string");
  if (files.length === 0) {
    env.__exportFiles = [];
    return;
  }
  if (!env.GITHUB_REPO) throw new Error("GITHUB_REPO env var is missing");
  if (!env.GITHUB_TOKEN) throw new Error("GITHUB_TOKEN env var is missing");
  ...
  const refRes = await countedFetch(`https://api.github.com/repos/${repo}/git/ref/heads/main`, { headers });
  ...
  await countedFetch(`https://api.github.com/repos/${repo}/git/refs/heads/main`, {
    method: "PATCH",
    headers,
...
try {
  await appendToBuffer(env, { path: charPath, content: toBase64Utf8(charContent) });
  await appendToBuffer(env, { path: indexPath, content: toBase64Utf8(indexContent) });
  try {
...
if (count >= GIT_BUFFER_FLUSH_THRESHOLD) {
  ...
  env.__exportFiles = allFiles.map(f => ({
  path: f.path,
  content: f.content
}));
await commitExport(env);
