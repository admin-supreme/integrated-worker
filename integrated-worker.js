...
async function commitExport(env) {
  ...
  const repo =
    env.GITHUB_REPO ||
    `${env.GITHUB_OWNER}/${env.GITHUB_REPO_NAME}`;
  if (!repo) throw new Error("Missing repo config");
  const branch = env.GITHUB_BRANCH || "main";
  await new Promise(r => setTimeout(r, 100));
  const files = env.__exportFiles || [];
  if (!files.length) return;
  const uniqueFiles = new Map();
  for (const file of files) uniqueFiles.set(file.path, file);
  const finalFiles = [...uniqueFiles.values()];
  const headers = {
  ...
  await ensureGithubOk(refRes, "ref fetch");
  ...
  await ensureGithubOk(commitRes, "commit fetch");
  const commitData = await commitRes.json();
  const baseTree = commitData.tree.sha;
  const treeItems = finalFiles.map(file => ({
    path: file.path,
    mode: "100644",
    type: "blob",
    content: String(file.rawContent)
  }));
  ...
  await ensureGithubOk(treeRes, "tree creation");
  ...
  const newCommitRes = await countedFetch(
    `https://api.github.com/repos/${repo}/git/commits`,
    {
      ...
      headers,
      body: JSON.stringify({
        message: `Auto export dataset (${finalFiles.length} files)`,
        tree: treeData.sha,
        parents: [latestCommitSha]
      })
    }
  );
  await ensureGithubOk(newCommitRes, "commit creation");
