...
for (const file of env.__exportFiles) {
    const path = file.path;
    if (!path) continue;
    const maybeBase64 = String(file.content || "");
    const isBase64 = /^[A-Za-z0-9+/=\r\n]+$/.test(maybeBase64);
    const contentBase64 = isBase64 ? maybeBase64.replace(/\s+/g, "") : toBase64Utf8(maybeBase64);
    let existingSha = undefined;
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        guard(); 
        const getUrl = `https://api.github.com/repos/${repo}/contents/${encodeURIComponent(path)}?ts=${Date.now()}`;
        const getRes = await countedFetch(getUrl, { headers: headersBase });
        if (getRes.status === 200) {
          const jd = await getRes.json();
          existingSha = jd.sha;
        } else if (getRes.status === 404) {
          existingSha = undefined;
        } else {
          const txt = await getRes.text().catch(() => "<no-body>");
          throw new Error(`GET contents failed (status ${getRes.status}): ${txt}`);
        }
        break;
      } catch (err) {
        console.warn(`GET attempt ${attempt} for ${path} failed:`, err.message || err);
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      try {
        ...
        let finalContent = contentBase64;
if (file.append && existingSha) {
  const getRes = await countedFetch(
    `https://api.github.com/repos/${repo}/contents/${encodeURIComponent(path)}`,
    { headers: headersBase }
  );
  if (getRes.status === 200) {
    const data = await getRes.json();
    const existing = decodeBase64Utf8(data.content || "");
    const incoming = decodeBase64Utf8(contentBase64);
    finalContent = toBase64Utf8(existing + incoming);
    existingSha = data.sha;
  }
}
const body = {
  message: `Auto export dataset: ${path}`,
  content: finalContent
};
if (existingSha) body.sha = existingSha;
        if (existingSha) body.sha = existingSha;
        const putRes = await countedFetch(putUrl, {
          method: "PUT",
          headers: headersBase,
        ...
        if (!putRes.ok) {
          const status = putRes.status || "no-status";
          throw new Error(`PUT failed (status ${status}): ${text.substring(0, 2000)}`);
        }
        console.log(`commitExport success: ${path} committed`);
        putSucceeded = true;
        break;
      } catch (err) {
        console.warn(`PUT attempt ${attempt} for ${path} failed:`, err.message || err);
        if (attempt === MAX_ATTEMPTS) {
          console.error(`Giving up on ${path} after ${MAX_ATTEMPTS} attempts`);
          throw err;
        }
    ...
    // small pause between files to be friendlier to rate limits
    await sleep(250);
  }

  // clear export buffer after success
  env.__exportFiles = [];
}
...
await appendToBuffer(env, {
  path: indexPath,
  content: toBase64Utf8(indexEntry),
  append: true
});
  try {
