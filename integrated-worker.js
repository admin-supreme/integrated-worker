...
try {
  ...
  if (isFlushMode) {
    ...
    if (env.__indexMap) {
  for (const [path, content] of Object.entries(env.__indexMap)) {
    allFilesFull.push({ path, content });
  }
}
...
if (!env.__indexMap) env.__indexMap = {};
env.__indexMap[indexPath] = toBase64Utf8(indexContent);
  try {
