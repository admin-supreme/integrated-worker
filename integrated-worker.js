...
try {
  ...
  if (bufCount >= GIT_BUFFER_FLUSH_THRESHOLD) {
    console.log(`FLUSH MODE: buffer=${bufCount}`);
    ...
    console.log("Flush complete. Buffer cleared.");
    return;
  }
} catch (e) {
...
async function commitExport(env) {
  ...
  try {
    ...
    if (!newCommitRes.ok) throw new Error(`GitHub new commit failed: ${newCommitRes.status}`);
    const newCommit = await newCommitRes.json();
    if (!newCommit?.sha) throw new Error("GitHub new commit missing sha");

    // 6️⃣ Update main branch ref
    const refUpdate = await countedFetch(`https://api.github.com/repos/${repo}/git/refs/heads/main`, {
      method: "PATCH",
      headers,
      body: JSON.stringify({ sha: newCommit.sha })
    });
    if (!refUpdate.ok) throw new Error(`GitHub ref update failed: ${refUpdate.status}`);

    console.log(`commitExport success: ${files.length} files committed`);
  } catch (err) {
    console.error("commitExport error:", err);
    throw err;
  } finally {
...
let existingRows = [];
if (INDEX_CACHE[indexPath]) {
  existingRows = INDEX_CACHE[indexPath];
} else {
  try {
