...
async function saveProgressId(env, memory = null) {
  ...
  const currentAnimeId = toPositiveInt(source.id);
  ...
  try {
    ...
    existing = {
      animeId: toPositiveInt(existingAnimeIdRaw),
...
async function readProgressState(env) {
  ...
  return {
    animeId: toPositiveInt(animeIdRaw),
