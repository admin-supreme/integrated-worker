...
async function processCharactersBatch(body, env) {
  ...
  if (env.__exportFiles && env.__exportFiles.length > 0) {
  try {
...
const res = await countedFetch(url, {
  ...
  env.__exportFiles.push({
    path: charPath,
    rawContent: charContent
  });
  env.__exportFiles.push({
    path: indexPath,
    rawContent: indexContent
  });
  if (env.__exportFiles.length >= COMMIT_THRESHOLD) {
    try {
      await commitExport(env);
    } catch (err) {
      if (err && err.message && err.message.includes("SAFE_STOP")) throw err;
      console.error("Buffered commit failed inside single payload:", err);
    }
  }
} catch (err) {
  if (err && err.message && err.message.includes("SAFE_STOP")) throw err;
  console.error("Error buffering character files:", err);
}
  if (body.last === true) {
    try {
