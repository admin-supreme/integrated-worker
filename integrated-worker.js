...
if (!env.GITHUB_REPO) {
  if (!env.GITHUB_OWNER || !env.GITHUB_REPO) {
    throw new Error("Missing env.GITHUB_REPO (expected 'owner/repo') or GITHUB_OWNER+GITHUB_REPO");
  }
}
const repo = env.GITHUB_REPO && env.GITHUB_REPO.includes("/") ? env.GITHUB_REPO : `${env.GITHUB_OWNER}/${env.GITHUB_REPO}`;
...
const headers = {
  ...
  "User-Agent": "Cloudflare-Worker-Database-Sync",
  ...
  if (!refRes.ok) {
    const text = await refRes.text().catch(() => "<no-body>");
    throw new Error("GitHub ref fetch failed: " + text);
  }
  const refData = await refRes.json();
  const latestCommitSha = refData.object.sha;
  const commitRes = await countedFetch(
    `https://api.github.com/repos/${repo}/git/commits/${latestCommitSha}`,
    { headers }
  );
  if (!commitRes.ok) {
    const text = await commitRes.text().catch(() => "<no-body>");
  ...
  const treeRes = await countedFetch(
  ...
  if (!treeRes.ok) {
    const text = await treeRes.text().catch(() => "<no-body>");
    throw new Error("GitHub tree creation failed: " + text);
  }
  const treeData = await treeRes.json();
  const newCommitRes = await countedFetch(
    `https://api.github.com/repos/${repo}/git/commits`,
    {
      method: "POST",
      headers,
      body: JSON.stringify({
        message: `Auto export dataset (${files.length} files)`,
        tree: treeData.sha,
  ...
  if (!newCommitRes.ok) {
    const text = await newCommitRes.text().catch(() => "<no-body>");
    throw new Error("GitHub commit creation failed: " + text);
  }
  const newCommit = await newCommitRes.json();
const refUpdate = await countedFetch(
...
await ensureGithubOk(refUpdate, "ref update");
  if (!refUpdate.ok) {
    const text = await refUpdate.text().catch(() => "<no-body>");
    throw new Error("GitHub ref update failed: " + text);
  }
  env.__exportFiles = [];
  console.log("Committed", files.length, "files to repo (batched).");
}
...
if (!repo) {
  ...
  if (!existingRows) {
    try {
if (!repo) {
  throw new Error("Missing GITHUB_REPO (owner/repo) or GITHUB_OWNER+GITHUB_REPO in env");
}
...
try {
  await appendToBuffer(env, { path: charPath, rawContent: charContent });
  await appendToBuffer(env, { path: indexPath, rawContent: indexContent });
  try {
    const count =
  (env.__kvBuffer ? env.__kvBuffer.length : 0) +
  (await getBufferCount(env));
if (count % 50 === 0) {
  console.log(`Buffered files count: ${count}`);
}
if (count >= GIT_BUFFER_FLUSH_THRESHOLD) {
  console.log("Flushing buffer to GitHub now...");
  const allFiles = env.__kvBuffer || [];
  env.__exportFiles = allFiles;
  await commitExport(env);
  await clearBuffer(env);
  env.__kvBuffer = [];
  console.log("Buffer committed and cleared.");
}
  } catch (e) {
  }
} catch (err) {
