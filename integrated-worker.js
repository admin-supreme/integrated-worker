...
async function runScheduled(env, db) {
  let isFlushMode = false;
  processedChunks = 0;
  let lastId = (await getAnimeProgress(env)) || 0;
try {
  const kvCount = await getBufferCount(env);
const memCount = env.__kvBuffer ? env.__kvBuffer.length : 0;
const totalBuffer = kvCount + memCount;
if (totalBuffer >= GIT_BUFFER_FLUSH_THRESHOLD) {
  isFlushMode = true;
}
    console.log(`FLUSH MODE: buffer=${totalBuffer}`);
...
const MAX_SKIP = 12;
if (isFlushMode) {
  console.log("Skipping fetch due to flush mode");
  return;
}
